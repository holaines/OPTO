...
== Functional design of the conditioning circuit

Before implementing the circuit, we estimated the main design values for the analog MEMS microphone and the INA131 conditioning stage. The sensitivity of the microphone is $-38$ dB re 1 V/Pa, which is equivalent to approximately 12.6 mV/Pa.

For an acoustic level of 80 dB SPL, the acoustic pressure is approximately 0.2 Pa. Therefore, the expected microphone output is:

$ V_m = 12.6 "mV/Pa" dot 0.2 "Pa" approx 2.5 "mV" $
...
#figure(
  image(img_path + "circuito.jpeg", width: 40%),
